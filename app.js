    ...
    for(let escNumb = curNumb; escNumb < starterValue.length; escNumb++ ){
       ...
       if(curNumb == 0){
        ...
        for(let opr = 0; opr < starterValue.length; opr++){
            ...
            if(isDuplicate){
                const numberOfDuplicate = searchDuplicateOnList(starterValue[opr]);
                const numberOfDuplicateonExpression = searchDuplicateOnExpression(starterValue[opr], curExprs);
                // compare if the number of value that have duplicate on list is more than already on current expressions  
                if(numberOfDuplicateonExpression.length >= numberOfDuplicate.length){
                    return;
                }else{
...
function searchDuplicateOnList(num){
    return starterValue.filter(item =>{
        return item == num;
    })
}
function searchDuplicateOnExpression(target, origin){
    let results =[];
